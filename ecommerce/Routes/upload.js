...
const fs = require('fs') 
// we will upload image in coludinary
cloudinary.config({
...
router.post('/upload',(req,res)=>{
    try{
        ...
        if(file.size > 1024*1024){
            ...
            return res.status(400).json({msg:"File size is greater than 1mb"})
        }if(file.mimetype !== 'image/jpeg' && file.mimetype !== 'image/png'){
            removeTemp(file.tempFilePath)
            return res.status(400).json({msg:"file  not jpeg or png type"})
        }
